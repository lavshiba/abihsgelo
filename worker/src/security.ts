...
export const TRANSITION_PASSWORD_HASH_SCHEME = "scrypt_v2";
...
export async function hashPasswordWithScheme(password: string, salt: string, pepper: string, scheme: string): Promise<string> {
  ...
  if (scheme === TRANSITION_PASSWORD_HASH_SCHEME) {
    return hashPasswordTransition(password, salt, pepper);
  }
...
async function hashPasswordTransition(password: string, salt: string, pepper: string): Promise<string> {
  const normalized = normalizePassword(password);
  const bytes = await scryptAsync(utf8ToBytes(`${normalized}:${pepper}`), hexToBytes(salt), {
    N: 1 << 12,
    r: 8,
